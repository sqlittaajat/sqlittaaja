...
def generate_html_table(similarity_results: dict[tuple[str, str], float], extracted: dict[str, str]) -> str:
    ...
    for student1_key, flags in similarity_results.items():
        ...
        for student2_key, similarity in flags:
            ...
            content1 = extracted[[key for key in extracted.keys() if key.startswith(student1_name)][0]]
            content2 = extracted[[key for key in extracted.keys() if key.startswith(student2_name)][0]]
            tables += make_table(content1.splitlines(),
                                 content2.splitlines(),
